...
";
                    ...
                    // 是否是关联属性
                    if (prop.Value.OneOf?.Count > 0 || prop.Value.Items?.Reference != null)
                    {
                        property.Reference = prop.Value.OneOf?.FirstOrDefault()?.Reference?.Id
                            ?? prop.Value.Items?.Reference?.Id;
                    }
                    else if (prop.Value.Reference != null)
                    {
