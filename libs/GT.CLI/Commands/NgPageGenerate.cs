...
namespace GT.CLI.Commands
{
    ...
    public class NgPageGenerate : GenerateBase
    {
        ...
        public void BuildAddPage()
        {
            ...
            // 解析属性，并生成相应代码
            var typeHelper = new TypeHelper(Path.Combine(ServicePath, "models", EntityName, EntityName + "AddDto.cs"));
...
";
        ...
        public void BuildEditPage()
        {
            ...
            tplContent = tplContent.Replace("{$EntityName}", EntityName)
                .Replace("{$EntityPathName}", EntityName.ToHyphen());
            // 解析属性，并生成相应代码
            var typeHelper = new TypeHelper(Path.Combine(ServicePath, "models", EntityName, EntityName + "UpdateDto.cs"));
            var props = typeHelper.PropertyInfos.Where(p => !p.IsList && !p.IsReference).ToList();
...
";
        ...
        public void BuildIndexPage()
        {
            ...
            var typeHelper = new TypeHelper(Path.Combine(ServicePath, "models", EntityName, EntityName + "Dto.cs"));
            // 需要展示的列
...
";
        ...
        public void BuildDetailPage()
        {
            ...
            {
                return $@"      <p>{p.Name}: <span>{{{{data.{p.Name.ToCamelCase()}}}}}</span></p>
";
